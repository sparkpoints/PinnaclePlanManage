...
import logging
import shutil
from Pinnutil import FTP_Module
...
def RetorePinnPatients(localDirPath, wantedFile, targetDirPath, loggerHand, ftpMode=True,):
    wantedList = []
    if ftpMode:
        ftpHandle = FTP_Module.ftpExt(FTP_IP)
        ...
        if curMRN in wantedList:
            ...
            if ftpMode:
                ...
                loggerHand.info("uploadFile(%d of %d):%s",
                                ind, len(wantedList), tarFile)
                ind += 1
                shutil.copy(os.path.join(localDirPath, tarFile), targetDirPath)
...
if __name__ == "__main__":
    ...
    #RetorePinnPatients(PINN_TRA_PATH, WANTED_LIST,LOCAL_DIR_PATH, logger, False)
