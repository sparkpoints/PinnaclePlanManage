...
import re
import random
from parse_single_pinn_file import parse_single_pinn_file
...
class tar_one_pinn_patient(object):
    def __init__(self, inst_head_template=None, target_dir=None):
        ...
        if inst_head_template is None:
            inst_head_template = os.path.join(os.getenv('HOME'),'PinnWork','institution_template')
        if not os.path.isdir(target_dir):
            print ("%s is not exist\n" % target_dir)
        else:
            patient_basic_info = self.get_patient_basic_infomation(target_dir)
            if patient_basic_info:
                tar_package_header = self.create_inst_header(inst_head_template,patient_basic_info)
                if tar_package_header:
                    self.tar_package_object = self.create_tar_file(patient_basic_info,tar_package_header,target_dir)
    ...
    def create_inst_header(self, inst_template, patient_info_object):
        ...
        patientData = patient_info_object.get_patient_data() #get patient basic infomation
    ...
    def get_patient_basic_infomation(self,data_dir):
        ...
        if os.path.isdir(data_dir) and os.path.isfile(os.path.join(data_dir, 'Patient')):
            patient_obj = parse_single_pinn_file(os.path.join(data_dir, 'Patient'))
    ...
    def create_tar_file(self, patient_info_object,tar_package_header,data_dir):
        ...
        #tarFile = patient_info['MedicalRecordNumber'] + patient_info['LastName'] + patient_info['FirstName'] + '_' +str(random.randint(1000,9999)) + '.tar'
        ...
        command = TAR + ' -c ' + ' --format=gnu ' + ' -f ' + tar_file_absPath + ' Institution '
        ...
        command = TAR + ' -r ' + ' --format=gnu ' + ' -f ' + tar_file_absPath + ' ' + patient_data_dir
        if not DEBUG:
            os.system(command)
...
if __name__ == "__main__":
    ...
    patient_file = os.path.join(work_path,"Patient_33140")
    obj3 = tar_one_pinn_patient(inst_template,patient_file)
    print obj3.get_tar_gzip_file()
